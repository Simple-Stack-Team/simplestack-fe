...
import { Assignment, EmployeeProject, TeamRole } from "@/app/[orgId]/dashboard/projects/[projectId]/types/teamfinder-types";
...
export const UpdateProposal = ({ assignment }: Props) => {
  ...
  const projectsUrl = `/organizations/${orgId}/projects/employee/${assignment.employee.id}`;
  const updateAssignmentUrl = `/organizations/${orgId}/projects/assignment-proposal/${assignment.id}`;
  ...
  const [teamRoles, setTeamRoles] = useState<string[]>(assignment.teamRoles)

  const form = useForm<z.infer<typeof FormSchema>>({
    resolver: zodResolver(FormSchema),
    defaultValues: {
      comments: assignment.comments,
      workHours: Number(assignment.workHours).toString(),
      teamRoles,
    },
  });
  ...
  const onSubmit = async (values: z.infer<typeof FormSchema> | any) => {
    ...
    await fetch(process.env.NEXT_PUBLIC_API_URL!+updateAssignmentUrl, {
      method: "PUT",
    ...
    toast({
      title: "Successfully updated proposed member",
      duration: 1500
    }))
    form.clearErrors()
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle className="mb-4 font-bold">Propose {assignment.employee.name}</DialogTitle>
        </DialogHeader>
        <Form {...form}>
